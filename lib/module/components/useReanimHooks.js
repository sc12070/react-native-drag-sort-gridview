...
var MOVEMENT;
(function (MOVEMENT) {
  MOVEMENT[MOVEMENT["restore"] = 0] = "restore";
  MOVEMENT[MOVEMENT["prev"] = 1] = "prev";
  MOVEMENT[MOVEMENT["next"] = 2] = "next";
})(MOVEMENT || (MOVEMENT = {}));
export default (_ref => {
  let {
    itemWidth,
  ...
  const animatedStyles = useAnimatedStyle(() => {
    ...
    return {
      transform: [{
        translateX: isDragging ? movementOffset.value.x : withTiming(movementOffset.value.x, {
          duration: animMoveDuration,
          easing: Easing.out(Easing.exp)
        })
      }, {
        translateY: isDragging ? movementOffset.value.y : withTiming(movementOffset.value.y, {
          duration: animMoveDuration,
          easing: Easing.out(Easing.exp)
        })
      }, {
